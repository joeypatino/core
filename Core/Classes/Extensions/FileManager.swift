...
public extension FileManager {
    ...
    var temporaryFileURL: URL {
        if #available(iOS 14.0, *) {
            return URL(fileURLWithPath: documentsDirectory.path, isDirectory: true).appendingPathComponent(UUID().uuidString).appendingPathExtension(for: .data)
        } else {
            var path = URL(fileURLWithPath: documentsDirectory.path, isDirectory: true).appendingPathComponent(UUID().uuidString)
            path.appendPathExtension("dat")
            return path
        }
    }
    
    var temporaryMovieURL: URL {
        if #available(iOS 14.0, *) {
            return URL(fileURLWithPath: cachesDirectory.path, isDirectory: true).appendingPathComponent(UUID().uuidString).appendingPathExtension(for: .quickTimeMovie)
        } else {
            var path = URL(fileURLWithPath: cachesDirectory.path, isDirectory: true).appendingPathComponent(UUID().uuidString)
            path.appendPathExtension("mov")
            return path
        }
    }
    ...
    var temporaryPhotoURL: URL {
        if #available(iOS 14.0, *) {
            return URL(fileURLWithPath: cachesDirectory.path, isDirectory: true).appendingPathComponent(UUID().uuidString).appendingPathExtension(for: .jpeg)
        } else {
            var path = URL(fileURLWithPath: cachesDirectory.path, isDirectory: true).appendingPathComponent(UUID().uuidString)
