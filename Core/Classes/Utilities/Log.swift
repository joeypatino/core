...
public struct Console {
    public enum Verbosity: CustomDebugStringConvertible {
        ...
        public static func ==(lhs: Verbosity, rhs: Verbosity) -> Bool {
            if case .trace = lhs, case .trace = rhs { return true }
    ...
    static var verbosity: [Verbosity] = [.trace, .info, .warn, .error, .verbose]
    ...
    public func trace(_ args: Any...) {
        _log(verbosity: .trace, args)
    }
...
private extension Bundle {
    static var mainBundleId: String { Bundle.main.bundleIdentifier ?? "com.joeypatino.logs" }
}
