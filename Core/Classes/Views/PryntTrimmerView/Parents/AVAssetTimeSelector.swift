...
public class AVAssetTimeSelector: UIControl, UIScrollViewDelegate {

    let assetPreview = AssetVideoScrollView()

    /// The maximum duration allowed for the trimming. Change it before setting the asset, as the asset preview
    public var maxDuration: Double {
        get { assetPreview.maxDuration }
        set { assetPreview.maxDuration = newValue }
    }
    ...
    public var asset: AVAsset? {
        ...
        set { newValue.map { imageGenerator = AVAssetImageGenerator(asset: $0) } }
    }

    public var imageGenerator: AVAssetImageGenerator? {
        didSet { imageGenerator.map { assetDidChange(newAsset: $0.asset) } }
    }
